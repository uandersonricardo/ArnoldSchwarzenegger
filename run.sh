...
uv run arnold.py --exp_name test --main_dump_path $PWD/dumped \
...
echo "Human player    : ${H}"
uv run arnold.py --exp_name test --main_dump_path $PWD/dumped \
--frame_skip 2 --action_combinations "attack+move_lr;turn_lr;move_fb" \
...
echo "Map ID          : 7"
echo "Human player    : ${H}"
uv run arnold.py --exp_name test --main_dump_path . \
