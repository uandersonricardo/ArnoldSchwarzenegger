...
class Scenario(Enum):
    ...
    FULL_DEATHMATCH = FullDeathmatch
...
def wrap_env(env: Env, map_id=1, **kwargs):
    env.unwrapped.game.set_doom_map(f'map{map_id:02d}')
...
def make(env_id, map_id=1, **kwargs) -> Env:
    return wrap_env(gymnasium.make(env_id, **kwargs.get('doom', {})), map_id=map_id, **kwargs.get('wrap', {}))
...
def make_level_fns(scenario: Scenario, level: int, **kwargs) -> List[Callable]:
    env_ids = get_env_ids(level, scenario)
    map_id = kwargs['wrap'].pop('map_id', 1)
    return [partial(make, env_id, map_id=map_id, **kwargs) for env_id in env_ids]
